...
> 版本：v1.0 ｜ 编制日期：2026-08-22 ｜ 用途：specc 框架选型与架构设计的规划输入
> 状态：规划阶段文档（不涉及编码）
...
| 维度 | Spec Kit | Kiro | BMAD | OpenSpec | Tessl |
|---|---|---|---|---|---|
| 出品方 | GitHub | AWS | 社区 | Fission AI | Tessl Inc. |
| 形态 | CLI + 模板 | 一体化 IDE/CLI | 多代理框架 | CLI + 斜杠命令 | 框架 + 规格注册表 |
| 开源 | ✅ MIT | ❌ | ✅ | ✅ MIT | ❌ |
| Agent 绑定 | 无（30+） | 自家（多模型） | 无（多工具） | 无（30+） | 自家生态 |
| 流程形状 | 线性 + 门禁 | 三文档栈线性 | 双阶段 + 角色流水线 | 变更状态机（可回退） | 规格 → 再生成 |
| 治理机制 | 宪法 | Steering + Hooks | 角色分工 + PO 审批 | 轻量约定 | 规格注册表 |
| 绿地/存量 | 绿地优 | 两者皆可 | 绿地优 | **存量优** | 绿地 |
| 重量级 | 中重 | 中 | 重 | **轻** | 重 |
| 需求表达 | 自由格式 + 验收标准 | **EARS（最规范）** | 用户故事 + AC | Requirement + Scenario | 结构化规格语言 |
| 可追溯性 | 任务分组 | **需求编号回链** | Story 回链 | 变更目录归档 | 规格即源 |
...
## 6. 对 specc 的启示（规划输入，待讨论）

结合我们的场景约束——**"Web 管理系统 + 微信小程序"双端、React+JS / Java 技术栈、Qwen 模型驱动**：

| 启示 | 说明 |
|---|---|
| **流程骨架取 Spec Kit** | 宪法 + specify/clarify/plan/tasks/implement 的线性门禁是被验证最充分的基线，治理感符合"合规"目标 |
| **需求表达取 Kiro EARS** | 结构化句式对中等能力模型（Qwen）尤其重要——格式约束能弥补推理不足 |
| **上下文分层取 BMAD 思路** | 宪法 / 平台层（双端通用规约）/ 业务层（项目领域知识）三层分离注入，避免全量塞入 |
| **任务粒度取"原子化 + 测试先行"** | 小任务短循环是对抗弱模型长程漂移的最有效手段 |
| **规格生命周期取 OpenSpec** | 变更归档进系统级规格库，解决长期维护中的规格漂移 |
| **验证环节双闸门** | 开发中自动检查（lint/测试/契约一致性）+ 交付前人工对照验收标准终审 |
| **引擎与规格解耦** | 规格层是纯 Markdown 资产，引擎（Codex Harness + Qwen）可替换 |
